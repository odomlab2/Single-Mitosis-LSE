...
all.snvs <- lapply(1:length(uv.gr), function(z){
  ...
  do.call(rbind, lapply(both.bases, function(x){
    ...
    # Reverse complement negative strand reads
    if(bam[[1]]$strand[[x]]=='+'){
...
for(dp.ex in 1:length(dual.alleles)){
  ...
  pheatmap(read.mat, cluster_rows = FALSE, cluster_cols = FALSE, color = c('white', 'grey', 'green', 'blue','red','orange'), 
           border_color = NA, legend=FALSE, main = names(dual.alleles)[dp.ex])
}
...
# Plot Panel 3b
plot(first.fork * 100, second.fork * 100, col=alpha(ifelse(is.mult,'red','black'),alpha = 0.7), pch=19, xlab='cC VAF', ylab='Cc VAF', 
     cex=1.3, xlim=c(0,100), ylim=c(0,100))
...
# Read representation across dual mutations with different VAFs
...
# Vectors of first and second alternate alleles
