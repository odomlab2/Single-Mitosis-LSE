...
sis1.gr <- rds.sis[[snp.match[clone]]]
...
# Adjust graph position to sjuxtaposed chromosomes
...
s1.is.c <- sis1.gr$REF=='C'
plot.colors.s1 <- adjustcolor(c(rep('dodgerblue4', sum(!(s1.is.c))), rep('goldenrod', sum((s1.is.c)))), alpha.f = 0.25)
...
plot(c(graph_pos_s1[!(s1.is.c)], graph_pos_s1[s1.is.c]), c(sis1.dist[!(s1.is.c)], sis1.dist[s1.is.c]), ylim=c(-10,10),
     col=plot.colors.s1, ylab='', pch=19, cex=pt.sz, xlab='position', main='', 
...
sis1.gr <- rds.sis[[snp.match[clone]]]
sis1.gr <- sis1.gr[((sis1.gr$REF=='C' & sis1.gr$ALT=='A') | (sis1.gr$REF == 'G' & sis1.gr$ALT=='T')) & sis1.gr$TimePoint=='T1']
...
segment.skews <- data.frame(do.call(rbind, lapply(names(rds.sis), function(x){
  ...
  cbind(skew.uv, uv.count, skew.ros, ros.count, rep(x, length(ros.count)))
...
# Density  plots of UV model
dens.uv <- density(uv.smth, bw=0.1)
plot(dens.uv, col='white', xlab='Mutation Skew', xlim=c(-1.3,1.3), main='Lesion Segregation')
polygon(dens.uv, col = alpha('darkgrey', alpha = 0.8), border=NA)

dens.ros <- density(ros.smth, bw=0.1)
plot(dens.ros, col='white', xlab='Mutation Skew', xlim=c(-1.3,1.3), main='No Lesion Segregation')
...
plot(dens.uv, col='white', xlab='', xlim=c(-1.3,1.3), main='', cex.axis=label.size, cex.lab=label.size, ylab='')
polygon(dens.uv, col = alpha('darkgrey', alpha = 0.8), border=NA)

hist(as.numeric(segment.skews[as.numeric(segment.skews[,'UV_count']) >= 10,'Skew_UV']), breaks=20, xlab='', ylab='', main='', col=alpha('firebrick4', alpha=0.6), 
...
plot(dens.ros, col='white', xlab='', ylab='', xlim=c(-1.3,1.3), main='', cex.axis=label.size, cex.lab=label.size)
polygon(dens.ros, col = alpha('darkgrey', alpha=0.8), border=NA)

hist(as.numeric(segment.skews[as.numeric(segment.skews[,'ROS_count']) >= 10,'Skew_ROS']), breaks=10, xlab='', ylab='', main='', col=alpha('cyan4', alpha=0.5), cex.axis=label.size, 
...
ros.uv.comp <- qqplot(as.numeric(segment.skews[as.numeric(segment.skews[,'ROS_count']) >= 10,'Skew_ROS']),  
                      as.numeric(segment.skews[as.numeric(segment.skews[,'UV_count']) >= 10,'Skew_UV']))

# Plot Panel 5e
par(mfrow=c(2,1))
qqplot(uv.exp, as.numeric(segment.skews[as.numeric(segment.skews[,'UV_count']) >= 10,'Skew_UV']),  pch=19, cex=0.8, 
       col=alpha('firebrick4', alpha=0.2), xlab='theoretical quartiles', ylab='sample quartiles')
...
qqplot(as.numeric(segment.skews[as.numeric(segment.skews[,'ROS_count']) >= 10,'Skew_ROS']),  ros.expected, pch=19, cex=0.8, 
       col=alpha('cyan4', alpha=0.2), xlab='theoretical quartiles', ylab='sample quartiles')
