...
par(mfrow=c(1,3)); y.limits <- c(500, 5000)
  plot(mut.counts[seq(2, nrow(mut.counts), by=3),], pch=19, col=c('dodgerblue4'), cex=1.7, xlim=y.limits, ylim=y.limits,
       xlab='sister 1 mutations', ylab='sister 2 mutations', main='ROS (G>T)')
    abline(0,1,lty=3, col='black', lwd=1.5)
  plot(mut.counts[seq(1, nrow(mut.counts), by=3),], pch=19, col=c('firebrick4'), cex=1.7, xlim=y.limits, ylim=y.limits,
       xlab='sister 1 mutations', ylab='sister 2 mutations', main='UV (C>T)')
    abline(0,1,lty=3, col='black', lwd=1.5)
  plot(mut.counts[seq(3, nrow(mut.counts), by=3),], pch=19, col=c('darkgrey'), cex=1.7, xlim=y.limits, ylim=y.limits,
       xlab='sister 1 mutations', ylab='sister 2 mutations', main='Other')
    abline(0,1,lty=3, col='black', lwd=1.5)
...
# Number of mutations unique, sister shared or clone shared 
overlap.stats <- do.call(rbind, lapply(1:length(ov.stats), function(x){
  message(x)
  tmp.df <- data.frame(ov.stats[[x]], stringsAsFactors = FALSE)
  
  c("unique" = sum(rowSums(tmp.df)==0)/nrow(tmp.df), 
    "sister shared"=sum(tmp.df[,overlap.pair[x]])/nrow(tmp.df), 
    "clone shared"=sum(rowSums(tmp.df[,grep(overlap.pair[x], colnames(tmp.df), invert=TRUE)])>0)/nrow(tmp.df))
...
# Color vector for reference base in mutation
...
for(i in 1:length(snp.list)){
  ...
  # Result variable and reverse complement slicing vector
...
axis(side=1, at=tick.marks, labels=c(0,25,50,75,100), tck=-0.06, padj = 1.25)
