...
# Mutations in signature, based on signature from Aitken 2020
...
# Plot Panel 7b
barplot(results/(sum(results)), col=col.trinuc, las=2, cex.names = 0.5, ylab='', 
        main='', border=NA, xaxt='n', ylim=c(-0.02,0.08), cex.axis=2)
...
skew.mat <- lapply(names(rds.cd), function(z){ 
  ...
  skew.tmp <- lapply(c('C3H', 'CAST', ''), function(sp){
    ...
    do.call(rbind, lapply(1:(length(ch.vec)), function(i){
      ...
      # Get granges and data frame. Select BL6 allele (check order by position)
...
# Adjusted genomic positions for concatenated chromsomes
...
for(spec.name in names(skew.mat[[tumor.sel]])){
  ...
  plot(pt.adj[is.a & spec.points], log10(snp.dist[(is.a & spec.points)]+1), pch=19, cex=0.4, col=point.colors[(is.a & spec.points)], frame.plot=FALSE, xaxt='n',
       ylab='distance to nearest (log10)', xlab='', ylim=c(2,6), cex.axis=1.5, xlim=c(1,sum(ch.vec)), main=spec.name)
  ...
  plot(pt.adj[(is.t & spec.points)], -1 * log10(snp.dist[(is.t & spec.points)]+1), pch=19, cex=0.4, 
       col=point.colors[(is.t & spec.points)], frame.plot=FALSE,xlim=c(1,sum(ch.vec)), ylab='distance to nearest (log10)', 
...
# Plot Panel 7e
barplot(100, col='darkgrey', ylim=c(0,100), border=NA, cex.axis=1.5)
...
for(spec.name in names(skew.mat[[tumor.sel]])[c(2,3,1)] ){
  ...
  plot(pt.adj[is.a & spec.points & chr.zoom], log10(snp.dist[(is.a & spec.points & chr.zoom)]+1), pch=19, cex=1.1, 
       col=point.colors[(is.a & spec.points & chr.zoom)], xaxt='n',
