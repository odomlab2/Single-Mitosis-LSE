...
# Bin X label text variable
med.pts <- unlist(lapply(split(mean.exp[(values(distanceToNearest(rna.cts))$distance >= 1000)], rna.cts$Exp_Bin), function(x){max(x)- ((max(x)-min(x))/2) }))
...
par(mar=c(1,4,2,1))
  ...
  hist(mean.exp[(values(distanceToNearest(rna.cts))$distance >= 1000)], main='', xlab='Bins', breaks=40, col='darkslategray4', border=NA, xaxt='n', ylim=c(-100,1500))
    axis(3)
  abline(v=c(0,quantile(mean.exp[some.exp], probs=seq(0,1,1/3))), col=2, lty=2, lwd=2)
  ...
  text(x = med.pts[1:2], y=-50, labels=c(1:2), cex=1.4, col='black')
  text(x = med.pts[3:4], y=-50, labels=c(3:4), cex=1.4, col='white')
...
# Minimum distance to nearest gene and minimum gene size
gene.dist <- 1000; gene.width <- 1000
...
genic.rates <- do.call(rbind, lapply(names(gene.tri), function(x){
  ...
  do.call(rbind, lapply(names(sister.split), function(xx){
    ...
    # Use both pairs, remove redundant mutations, filter on at least 10 reads
...
# Mean and standard deviation of rates
