...
# Mapping SNP objects to segmentation map
...
for(chrom in c('chr17', 'chr8', 'chr2')){
  ...
  plot(start(sis1.gr)/10^6, sis1.dist, ylim=c(-10,10), col=plot.colors.s1, ylab='', pch=19, cex=pt.sz, xlab='position', main='', cex.axis=1.5)
  abline(h=0, col=1, lty=3)
  plot(start(sis2.gr)/10^6, sis2.dist, ylim=c(-10,10), col=plot.colors.s2, ylab='', pch=19, cex=pt.sz, xlab='position', main='',  yaxt='n', cex.axis=1.5)
  abline(h=0, col=1, lty=3)
}
...
pt.sz <- 0.6
clr.trns <- 0.2
par(mfrow=c(2,1), mar=c(0.4,4,2,4))
...
plot(c(graph_pos_s2[!(s2.is.c)], graph_pos_s2[s2.is.c]), c(sis2.dist[!(s2.is.c)], sis2.dist[s2.is.c]), ylim=c(-10,10),
     col=plot.colors.s2, ylab='', pch=19, cex=pt.sz, xlab='position', main='', xlim=c(0,sum(ch.vec)),
     xaxt='n')
...
# Plot Panel 6d
pheatmap(seg.mat, color=segment.colors, show_colnames = FALSE, show_rownames=FALSE, border_color = NA, main='', 
         cluster_rows = FALSE, cluster_cols = FALSE, gaps_col  = chrom.breaks, gaps_row =seq(2,nrow(seg.mat), by=2), cellheight = 15, cellwidth=0.65)
...
##  Look at mutation density around an SCE
##---------------------------------
sce.slide <- Reduce('+', lapply(names(sce.map), function(x){
  ...
  # Order each SCE site so seperated strands are on the left
